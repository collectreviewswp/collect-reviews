...
import FormSelect from "components/integrations/forms/FormSelect";
import FormFieldSelect from "components/integrations/forms/FormFieldSelect";
...
const WPForms = () => {
	...
	let initialValues = data.options?.integrations?.wpforms || {
		enabled: true,
		triggers: []
	};
	...
	return (
		<>
			<PageHeader>
				<PageTitle>{__( 'WPForms', 'collect-reviews' )}</PageTitle>
							...
							<FieldArray name="triggers">
								{() => (values.triggers || []).map( ( v, triggerKey ) =>
									<React.Fragment key={triggerKey}>
										<SettingRow
											label={__( 'Form', 'collect-reviews' )}
											labelFor="form"
											description={__( 'Review requests will be created after the selected form submission.', 'collect-reviews' )}
											field={
												<FormSelect
													integration="wpforms"
													name={`triggers.${triggerKey}.form`}
													id="form"
													onChange={( event, value ) => {
														setFieldValue( `triggers.${triggerKey}.form`, value );
														setFieldValue( `triggers.${triggerKey}.form_email_field`, null );
													}}
												/>
											}
										/>
										...
										{v.form &&
											<>
												<SettingRow
													label={__( 'Form Email Field', 'collect-reviews' )}
													labelFor="form-email-field"
													description={__( 'A review request email will be sent to the email address from the selected field.', 'collect-reviews' )}
													field={
														<FormFieldSelect
															integration="wpforms"
															name={`triggers.${triggerKey}.form_email_field`}
															id="form-email-field"
															formId={v.form?.id}
															fieldType='email'
														/>
												...
												<SettingRow
													label={__( 'Form Name Field', 'collect-reviews' )}
													labelFor="form-name-field"
													optional
													description={__( 'Fill in this field if you want to use the customer\'s name in the review request email.', 'collect-reviews' )}
													field={
														<FormFieldSelect
															integration="wpforms"
															name={`triggers.${triggerKey}.form_name_field`}
															id="form-name-field"
															formId={v.form?.id}
															fieldType='name'
														/>
										...
										<SettingRow
											...
											description={__( 'The delay before the review request email will be sent after the form submission. It can be useful if you prefer to wait until the order has been delivered or the product has been used for some time before sending the review request email.', 'collect-reviews' )}
											field={
												<Select
