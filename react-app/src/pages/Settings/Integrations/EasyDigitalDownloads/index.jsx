...
const EasyDigitalDownloads = () => {
	...
	let initialValues = data.options?.integrations?.easy_digital_downloads || {
		enabled: true,
		triggers: []
	};
		...
		onSubmit: async ( values, {setSubmitting, setValues} ) => {
			...
			try {
				const updatedValues = await api.saveSettings( {
					integrations: {
						easy_digital_downloads: values
					}
				} );

				setValues( updatedValues.integrations.easy_digital_downloads || {} );

				// Update global options store.
				data.options = updatedValues;
				data.integrations = data.integrations.map( integration => {
					if ( integration.slug === 'easy_digital_downloads' ) {
	...
	const orderStatuses = data.integrations?.find( i => i.slug === 'easy_digital_downloads' )?.order_statuses || {};

	return (
		<>
			<PageHeader>
				<PageTitle>{__( 'Easy Digital Downloads', 'collect-reviews' )}</PageTitle>
...
export default EasyDigitalDownloads;
